...
for i in range(n):  # iteratively update frequency grid
    ...
    p_offset = phase_offset_rad[str(rr_no)]       
        # Define the model
    def model(x, A, B, C, D, e_delay, p_offset):
         # A and B give the amplitude of signal, C and D set frequency shift and correction
         # e_delay and p_offset are parameters we want to optimize 
        real = A * np.cos(2 * np.pi * (C + x * e_delay) + p_offset) + B
        imag = A * np.sin(2 * np.pi * (D + x * e_delay) + p_offset)
        return np.sqrt(real**2 + imag**2)

    # Set initial guesses for fitting
    A = np.real(sig[0])
    B = np.imag(sig[0])
    C = freq_list[np.argmin(sig)]
    D = freq_list[np.argmin(sig)]
    e_delay_start = e_delay       #init guess for e_delay
    p_offset_start = p_offset     #init guess for p_offset

    p0 = [A, B, C, D, e_delay_start, p_offset_start]
    popt, pcov = curve_fit(model, freq_list, sig_adj, p0=p0)

    # Optimized parameters are saved into popt, 
    # optimized e_delay and p_offset being last two
    e_delay_opt = popt[-2] # Here is optimized electrical delay
    p_offset_opt = popt[-1] # Here is optimized phase offset
    
    sig_corrected = sig_adj*np.exp(1j*2*np.pi*freq_list*e_delay_opt + 1j*p_offset_opt)
